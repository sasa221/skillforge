...
import type { AdminSkill } from '@/lib/content/types';
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {items.map((item) => (
        <span
          key={item}
          className="rounded-full border border-[var(--site-border)] bg-[var(--site-surface)] px-3 py-1 text-xs font-semibold text-[var(--site-text)]"
        >
          {labelFor(item)}
        </span>
      ))}
...
export function InstructorCourseEditClient({ id }: { id: string }) {
  const qc = useQueryClient();
  const { toast } = useToast();
  const course = useQuery({
    queryKey: ['instructor', 'courses', id],
    queryFn: () => instructorWorkspaceApi.courses.get(id),
  });
  const skills = useQuery({
    queryKey: ['instructor', 'skills'],
    queryFn: instructorWorkspaceApi.skills,
  });
  ...
  const modules = useQuery({
    ...
    queryKey: ['instructor', 'courses', id, 'modules'],
    queryFn: () => instructorWorkspaceApi.courses.modules(id),
  });
  ...
  const revisionFieldRenderers = React.useMemo(
    ...
    [mediaTitlesById, skillTitlesById],
  );

  const save = useMutation({
    mutationFn: async () =>
      instructorWorkspaceApi.courses.update(id, {
        ...form,
    ...
    onSuccess: async () => {
      await Promise.all([
        qc.invalidateQueries({ queryKey: ['instructor', 'courses', id] }),
        qc.invalidateQueries({ queryKey: ['instructor', 'workspace'] }),
      ]);
      toast({ title: 'Course saved', description: 'Your course updates were saved.' });
    },
    onError: (error) =>
      toast({
        title: 'Save failed',
  ...
  const restoreRevision = useMutation({
    mutationFn: (revisionId: string) => instructorWorkspaceApi.courses.restoreRevision(id, revisionId),
    onSuccess: async () => {
      await Promise.all([
        qc.invalidateQueries({ queryKey: ['instructor', 'courses', id] }),
        qc.invalidateQueries({ queryKey: ['instructor', 'courses', id, 'modules'] }),
        qc.invalidateQueries({ queryKey: ['instructor', 'workspace'] }),
      ]);
  ...
  const submitReview = useMutation({
    mutationFn: async () =>
      instructorWorkspaceApi.courses.submitReview(id, reviewDraftNote.trim() || undefined),
    onSuccess: async () => {
      await Promise.all([
        qc.invalidateQueries({ queryKey: ['instructor', 'courses', id] }),
        qc.invalidateQueries({ queryKey: ['instructor', 'workspace'] }),
      ...
      toast({
        ...
        description: 'The course is now waiting for admin review.',
      });
    },
    onError: (error) =>
      toast({
  ...
  const createModule = useMutation({
    mutationFn: async (title: string) =>
      instructorWorkspaceApi.courses.createModule(id, { title, status: 'draft' }),
    onSuccess: async () => {
      await Promise.all([
        qc.invalidateQueries({ queryKey: ['instructor', 'courses', id, 'modules'] }),
        qc.invalidateQueries({ queryKey: ['instructor', 'workspace'] }),
      ]);
      toast({ title: 'Draft module created', description: 'You can keep building the module flow from here.' });
    },
    onError: (error) =>
      toast({
        title: 'Could not create module',
        description: error instanceof Error ? error.message : 'Please try again.',
        variant: 'destructive',
      }),
  });
  ...
  return (
    <main className="space-y-6">
      <AdminPageIntro
        title="Manage Course"
        description="Refine the course outline, media, and learning flow for your assigned learners."
        actions={
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_420px]">
        <AdminSurface>
          <div className="grid gap-5 md:grid-cols-2">
            ...
            <AdminField label="Intro Video URL" className="md:col-span-2">
              <input
            ...
            <AdminField label="Tags" className="md:col-span-2">
              <input
                ...
                className={adminInputClassName}
                placeholder="excel, formulas, analytics"
              />
            </AdminField>
          </div>
        </AdminSurface>
        ...
        <div className="space-y-6">
          <AdminSurface>
            ...
            <div className="mt-4">
              <AdminStatusPill tone={statusTone(courseData.status)}>{courseData.status.toUpperCase()}</AdminStatusPill>
            </div>
            <div className="mt-4 text-sm leading-7 text-[var(--site-muted)]">
              Status is managed outside the instructor workspace. You can keep refining the content here while the course remains in its current state.
            </div>
          </AdminSurface>

          <ContentReviewWorkflowPanel
            audience="instructor"
            reviewStatus={courseData.reviewStatus}
            reviewNotes={courseData.reviewNotes}
          ...
          <ContentRevisionPanel
            ...
            revisions={courseData.revisions}
          ...
          <CourseLinkedAssetsPanel
            ...
            coverAsset={selectedCoverAsset as any}
            coverImageUrl={form.coverImageUrl}
            introVideoAsset={selectedIntroVideoAsset as any}
          ...
          <AdminSurface>
            ...
            {skills.isLoading ? (
              ...
              <div className="mt-4 flex flex-wrap gap-3">
                {(skills.data ?? []).map((skill: AdminSkill) => {
          ...
          {save.isError || restoreRevision.isError || createModule.isError || submitReview.isError ? (
            <div className="rounded-[1.2rem] border border-[var(--site-danger)]/20 bg-[var(--site-danger-soft)] px-4 py-3 text-sm text-[var(--site-danger)]">
              {save.error instanceof Error
                ? save.error.message
                : restoreRevision.error instanceof Error
                  ? restoreRevision.error.message
                  : createModule.error instanceof Error
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-base text-[var(--site-muted)]">
              Build the course sequence and add new draft modules from your workspace.
            </p>
          </div>
          <CreateInline
            label="New draft module"
  ...
  return (
    <div className="flex flex-wrap gap-3">
      <input
        ...
        className="h-12 min-w-[220px] rounded-[1rem] border border-[var(--site-border)] bg-[var(--site-surface-alt)] px-4 text-sm text-[var(--site-text)] outline-none transition placeholder:text-[var(--site-subtle)] focus:border-primary/35"
      />
      <button
        type="button"
        onClick={() => {
        ...
        disabled={disabled}
        className="inline-flex h-12 items-center justify-center rounded-[1rem] bg-primary px-4 text-sm font-semibold text-primary-foreground transition hover:bg-primary/90 disabled:opacity-60"
      >
        Create
      </button>
      <button
...
function statusTone(status: string): 'orange' | 'emerald' | 'violet' | 'blue' | 'slate' {
