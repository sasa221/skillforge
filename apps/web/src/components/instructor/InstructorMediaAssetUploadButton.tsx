...
export function InstructorMediaAssetUploadButton({ kind, onUploaded, className }: Props) {
  ...
  const upload = useMutation({
    mutationFn: async (file: File) =>
      instructorWorkspaceApi.uploadMediaAsset({
        file,
    ...
    onSuccess: async (asset) => {
      await queryClient.invalidateQueries({ queryKey: ['instructor', 'media-assets'] });
      onUploaded(asset);
      toast({
        title: `${kind === 'image' ? 'Image' : 'Video'} uploaded`,
        description: `${asset.title} is ready for this course.`,
      });
    },
    onError: (error) =>
      toast({
